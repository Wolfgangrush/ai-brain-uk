...
# ─────────────────────────────────────────────────────────────────────────
# Regex patterns — UK-relevant government IDs + structured identifiers (NI · UTR · etc. layered with India patterns kept for cross-border matters)
# ─────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────
# UK-native PII patterns (added v0.1.1 for jurisdictional coverage)
# ─────────────────────────────────────────────────────────────────────────

# NI Number: 2 letters + 6 digits + 1 letter (e.g. AB123456C)
NI_NUMBER_RE = re.compile(r"\b[A-CEGHJ-PR-TW-Z]{1}[A-CEGHJ-NPR-TW-Z]{1}\d{6}[A-D]{1}\b")

# NHS Number: 10 digits, often grouped 3-3-4
NHS_NUMBER_RE = re.compile(r"\bNHS[\s:#]*\d{3}\s?\d{3}\s?\d{4}\b", re.IGNORECASE)

# UTR (Unique Taxpayer Reference): 10 digits, often preceded by UTR
UTR_RE = re.compile(r"\bUTR[\s:#]*\d{10}\b", re.IGNORECASE)

# UK VAT Number: GB + 9 or 12 digits
UK_VAT_RE = re.compile(r"\bGB\d{9}(?:\d{3})?\b")

# UK phone: +44 prefix or 07/01/02 formats
UK_PHONE_RE = re.compile(r"(?:\+?44[\s-]?)?(?:0?(?:7\d{3}|[12]\d{3}))[\s-]?\d{3}[\s-]?\d{3}\b")

# GBP amounts: £
GBP_AMOUNT_RE = re.compile(r"\(?-?\s?£\s?-?\d{1,3}(?:,\d{3})*(?:\.\d+)?\)?")

# UK case numbers: [YYYY] EWHC/EWCA/UKSC NNN (Civ/Crim/Comm/etc)
# UK postcode: e.g. SW1A 2AA, M1 1AA, EC1A 1BB
UK_POSTCODE_RE = re.compile(r"\b[A-Z]{1,2}\d[A-Z\d]?\s*\d[A-Z]{2}\b")

UK_CASE_RE = re.compile(
    r"\[?\d{4}\]?\s+(?:EWHC|EWCA|UKSC|UKHL)\s+(?:Civ|Crim|Comm|Admin|Fam|TCC|Ch|QB)?\s*\d{1,4}",
    re.IGNORECASE,
)

# UK IBAN: GB + 2 check digits + 18 alphanumeric (22 chars total)
...
class PseudonymisationGateway:
    ...
    def __init__(self) -> None:
        ...
        self.patterns: list[tuple[re.Pattern, str]] = [
            # UK-native (priority for UK jurisdiction)
            (UK_POSTCODE_RE, "UK_POSTCODE"),
