...
> **Free practice OS for every UK solo solicitor, barrister, advocate (Scotland), CILEx-regulated practitioner, and in-house counsel. Terminal-native. Cloud-LLM behind the [Pseudonymisation Gateway](https://github.com/Wolfgangrush/pseudonymisation-gateway) sanitising PII before any prompt leaves the machine.**
...
**Version:** 2.0.0 · **License:** MIT · **Publisher:** [wolfgang_rush](https://github.com/Wolfgangrush) — an Indian advocate (High Courts of India, India). NOT admitted in any UK jurisdiction. This is a software publication for UK-admitted practitioners. · **Engine:** Built on [MemPalace](https://github.com/MemPalace/mempalace) (MIT)
...
- Unified the ChromaDB collection name across the MCP server and the CLI/search paths (all now read it from `BrainConfig().collection_name`). Drawers saved via the MCP server are now findable by search when a custom `collection_name` is configured.
- Removed dead code — an uncalled knowledge-graph seeder that referenced a module that no longer exists, and a no-op statement in the entity detector.
- Consolidated two duplicate stop-word lists into one shared `stopwords` module imported by both the entity detector and the dialect compressor, so they can no longer drift apart.
