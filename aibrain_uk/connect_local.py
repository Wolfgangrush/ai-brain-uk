...
Usage from CLI:
    ...
    aibrain-uk connect-local --yes      # skip prompts
    aibrain-uk connect-local --model qwen3:7b   # force a specific model

Result: ~/.aibrain-uk/config.json is written with ai_provider=ollama and the
...
def install_ollama_instructions() -> None:
    ...
    print()
    print("Then run `aibrain-uk connect-local` again.")
...
def cmd_connect_local(args) -> int:
    ...
    print("  aibrain-uk connect-cloud --provider deepseek --cloud-warning-acknowledged")
    print()
    return 0
