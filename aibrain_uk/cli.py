...
Commands:
    aibrain-uk reception            Turn it on: greet, verify specialists, load memory, show recap
    aibrain-uk ask "<question>"     One-shot — route a single question to the right specialist
    aibrain-uk chat                 Interactive session — every line is routed for you
    aibrain-uk recap                Show the retrospective memory of recent sessions
    aibrain-uk mine <dir>           File project files / conversation exports into the palace

Examples:
    aibrain-uk reception
    aibrain-uk ask "limitation for breach of contract filed 12 Jan 2026"
    aibrain-uk ask "which court has jurisdiction over my matter"
    aibrain-uk chat
    aibrain-uk mine ~/chats/claude-sessions --mode convos
"""
...
# ---------------------------------------------------------------------------
# Terminal brain front-door commands
# ---------------------------------------------------------------------------
# These route a query through the brain (classify -> route -> agent ->
...
def cmd_chat(args):
    ...
    from .brain.repl import run_chat

    return run_chat()


def cmd_reception(args):
    """Boot the brain: greet, verify specialists, turn on memory, show recap."""
    from .brain.reception import run_reception
...
def cmd_recap(args):
    ...
    from .brain.memory import recap
...
def cmd_compress(args):
    ...
    if not args.dry_run:
        try:
            comp_col = client.get_or_create_collection("aibrain_uk_compressed")
            ...
            print(
                f"  Stored {len(compressed_entries)} compressed drawers in 'aibrain_uk_compressed' collection."
            )
        except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--palace",
        default=None,
        help="Where the palace lives (default: from ~/.aibrain-uk/config.json or ~/.aibrain-uk/palace)",
    )
    ...
    p_ask = sub.add_parser(
        "ask",
        help="Ask the brain one question — it routes to the right specialist (UK · England & Wales)",
    )
    p_ask.add_argument(
        "query",
        nargs="+",
        help='Your question, e.g. "limitation for breach of contract filed 12 Jan 2026"',
    )
    ...
    sub.add_parser(
        "reception",
        help="Boot the brain: greet, verify specialists, turn on memory, show recap",
    )
