...
from aibrain_uk.brain import memory


# Specialist modules present in aibrain_uk/agents/. Kept in sync with
...
def run_reception() -> int:
    ...
    for name in AGENTS:
        try:
            mod = importlib.import_module(f"aibrain_uk.agents.{name}")
    ...
    router_ok = False
    try:
        router = importlib.import_module("aibrain_uk.brain.router")
    ...
    print(f"  brain: ON  ·  specialists: {online}/{len(AGENTS)} online  ·  router: {router_state}")
    print()
